...
export function summarizeUsage(days = 7) {
  ...
  const totals = { requests: 0, input: 0, output: 0, total: 0, errors: 0 };
  ...
  for (const e of events) {
    ...
    const m = byModel.get(mKey) || { model: e.model || '未知模型', provider: p.name, requests: 0, input: 0, output: 0, total: 0 };
    m.requests += 1;
    m.input += input;
    m.output += output;
    m.total += total;
